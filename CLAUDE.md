...
**jig** は jq 互換の JSON プロセッサ CLI。jq の不評部分（診断・数値精度・
パーサクラッシュ）を直しつつ、既存の jq プログラムがそのまま動くことを
目指す。**互換性の規範は [docs/jq-compat.md](docs/jq-compat.md)** —
「jq とどこまで同じか / どこを変えてよいか」の仕様判断はすべてそこに
集約する（このファイルや PR 議論に分散させない）。
