...
[docs/roadmap.md](docs/roadmap.md)**、**言語設計原則の正本は
[docs/principles.md](docs/principles.md)**（Unix-native・comma は常にストリーム・jq を
忘れる・小さく重ねる・診断＝製品）— まずこの2つを読む。
...
> 以前の「jq 互換契約 / dual-mode / `--humane` / `alias jq=jig`」は撤去済み
> （意味論は一つ。コードからも削除）。[docs/jq-compat.md](docs/jq-compat.md)
> は **SUPERSEDED**（歴史的参考）— 差分カタログ・診断哲学の資料として残す。
...
- **JigCore に Foundation を足さない**（Log.swift の既存例外を除く）。
  ...
  （[docs/roadmap.md](docs/roadmap.md)）。`String(format:)` の代わりに
...
- **意味論は一つ（dual-mode は撤去済み）**: jig に jq/humane の切替は無い。
  `.[]` を null に適用すると **常に空ストリーム**（旧: エラー）、`//` は
  **常に** false+null を捨て（jq 同様）、`??` は nullish（null のみ捨て、
  false は残す）— `//` と `??` は意図的に別物。`--humane` flag /
  `# jig:humane` pragma / `JIG_MODE` env var / `JigMode` enum /
  `Sources/JigCore/Mode.swift` は**すべて削除済み**。正本は
...
- **YAML / TOML 入力**: 当面しない（yq の領分）。やるなら別 frontend として
  起票してから。
- **意味論は一つ**: jig に mode 切替は無い。破壊的な意味論変更は
  ロードマップに沿って許可される（mode トグルは存在しない）。判断・方針の
  正本は [docs/roadmap.md](docs/roadmap.md)。
...
filter (v0 subset — 言語ロードマップは docs/roadmap.md):
...
- [docs/roadmap.md](docs/roadmap.md) — このリポジトリの方向性の正本（意味論・言語モデル・スコープ）
