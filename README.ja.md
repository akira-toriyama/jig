...
filter(f) has(k)` ＋ Wave 1 合成セット
`range(n) groupBy(f) mapValues(f) orderBy(f) toPairs fromPairs`
（`countBy` は `groupBy(f) | mapValues(length)`、降順は `orderBy(f) | reverse`）
（jq alias `type` / `add` / `select` / `map_values` も受理）。subcommand
