...
- **▶ 次セッションの起点（実装）**: **§5 (5) の次の Wave1 サブステップ** = `countBy`/`keyBy`/`sumBy`/`min`/`max` 等を ↑ 合成セットの上に composable に乗せる（`countBy` は既に `groupBy \| mapValues(length)` で成立＝専用 builtin は任意）。または **§5 (6) 補完エンジン `jig complete`（B＝データ駆動）**。検証は `swift build` + 実機バイナリ → CI で XCTest（push→PR→CI green→squash-merge）。基盤として §5 (12) 評価器 lazy 化（`range` 無限・`first`/`limit` 解禁）。
