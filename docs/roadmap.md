...
5. ✅ **Wave1 合成セットで「小さく重ねる」を E2E 成立**（決定③・done 2026-06-14）: 配列/文字列スライス `.[a:b]`（AST `.slice` + parser + eval + explain）＋ `range`(eager 有限・上限ガード) / `groupBy`(オブジェクト返し) / `mapValues`(空出力で key drop) / `orderBy`(comma-tuple 多キー・安定・`"desc"` 文字列リテラルを診断) / `toPairs`・`fromPairs`。`countBy` = `groupBy | mapValues(length)` が成立。`map_values`→`mapValues` だけ alias 追加（`group_by`/`to_entries`/`sort_by` は別形/別名 ＝ 非 alias）。正典化パターン（§5-4・`canonicalBuiltinName` + evalCall switch）に乗せた。実行プラン: [docs/plan-wave1.md](plan-wave1.md)。
   ...
   - ✅ **Wave1 平均＋整形サブステップ**（done 2026-06-14・PR #21）: `mean`/`meanBy`(空→null・非数値エラー・alias `avg`/`avgBy`)・`pick(keys)`(キー文字列選択・要求順・欠損スキップ・**jq の path 版 `pick(pathexp)` とは別シェイプ**)・`omit(keys)`(キー落とし・入力順保持)。`pick`/`omit` のキーは comma-ストリーム（`pick("a","b")` / 動的 `pick(.f[])`）。`groupBy(.c)|mapValues(meanBy(.price))` ＝ カテゴリ別平均が成立。
   - ✅ **`pickBy`/`omitBy`**（done 2026-06-14・PR #22）: `filter` のオブジェクト版＝各値に述語 f（`.`＝値）を適用し真の entry を残す/落とす（順序保持・空出力 falsy・非 object エラー）。`omitBy(. == null)` で null 一掃。jq の `with_entries(select)` の優しい形・非 alias。
   - ✅ **`reduce` ＋ `$variable` 束縛**（done 2026-06-14・PR #23）: `reduce SOURCE as $x (INIT; UPDATE)` ＝ ストリームを累積値へ畳む。**INIT の各出力ごとに独立 fold を1回**（空 INIT→出力なし・N 出力→N 結果）、各 fold 内で SOURCE 値ごとに `$x` 束縛→UPDATE を累積値 `.` 上で評価しその**最後の出力**を新累積値（空→null）。**jq バイト一致を実機確認**（multi/empty INIT 含む。※敵対レビューが「INIT 第1出力のみ」の初版バグを検出→修正・テスト追加）。**実装は環境スレッディングでなく `substitute`（AST 代入）**＝評価器 hot path 不変・レキシカルシャドウも正しい（ネスト同名 reduce 検証済）。`$x` は新 AST `.variable`、reduce 外は「未束縛」エラー。parser に `$ident`＋`parseReduce`。**Wave1 builtin/言語章はこれで完了。** 注: `.[$x]` 動的 index と `+=`/`|=` 更新代入は未対応（jq の object-accumulate は書けない・別 roadmap）・外部変数 `--arg`/`--argjson` も未 [[project_jig_zip_gap]]。
...
- **済（2026-06-14 ＝ §5(5) 残り builtin・PR #22 `pickBy`/`omitBy`・PR #23 `reduce`＋`$variable` 束縛）**: 詳細は §5(5) の ✅ 群。**Wave1 builtin/言語章 完了**。`reduce` は `substitute`(AST 代入)で実装＝評価器 hot path 不変。
- **▶ 次セッションの起点（実装）**: **§5 (6) 補完エンジン `jig complete`（B＝データ駆動）**（prefix を入力に部分評価→実フィールド名→対話 C `jig -i`→静的 A）が本命＝戦略的差別化。または小粒の残りカタログ §3（`flatten`/`chunk`/集合演算/型述語 `isString` 等/`get(path;default)`）や `.[$x]` 動的 index＋`+=`/`|=` 更新代入（path エンジン）。検証は `swift build` + 実機バイナリ → CI で XCTest（push→PR→CI green→squash-merge）。基盤として §5 (12) 評価器 lazy 化（`range` 無限・`first`/`limit` 解禁）。
