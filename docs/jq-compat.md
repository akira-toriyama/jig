...
| 8 | 大整数が silently 壊れる（≤1.6）、1.7 でも演算で壊れる | **[修正]** literal 保存は **実装済み (v0)**（`JigNumber.literal`）。算術 (`+ - * / %`) は **実装済み (v0)**: jq 1.7 と同じく未演算リテラルのみ保存し、演算は double 化（観測動作が jq mode と一致）。gojq 方式の任意精度整数は jq と乖離するので **humane 拡張**として roadmap（採用時はモード差分表に 1 行追加） |
...
2. **scalar リテラル（number/string/bool/null）実装済み (v0)**。
   **object / array construction 実装済み (v0)**: `{a: .b}`、短縮形 `{user}`
...
| `${...}` template literal | 文字列補間 `\(…)` の同義として `"${expr}"` を受理 | [追加] | roadmap 2 |
