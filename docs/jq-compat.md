...
> ⚠️ **2026-06-13 以降 SUPERSEDED**: jig は jq の**厳密互換を追わなくなった**。
> 方向性の正本は **[docs/roadmap.md](roadmap.md)**（jig = 人に優しい JSON 操作
> CLI、Unix 純パイプ + lodash 語彙 + humane 診断）。本ファイルの「jq モードは
> 決して乖離しない」契約は無効。ただし **差分カタログ・診断哲学・null 挙動表・
> ECMAScript エルゴノミクス表は参考資料として有効** なので残す。
